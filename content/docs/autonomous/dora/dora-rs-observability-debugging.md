...
노드별 큐 길이와 처리 지연을 확인할 수 있는 메트릭 출력을 지원하는 버전에서는, `brightness` 노드의 큐 길이가 계속 늘어나는지를 여기서 확인합니다. 큐가 계속 쌓이고 있다면 원인은 보통 셋 중 하나입니다.
...
```bash
dora logs brightness --since "2026-07-14T10:03:12"
```
