...
로그와 메트릭은 "노드가 실행되고 있는가", "큐가 쌓이는가"는 알려주지만, "지금 흐르고 있는 데이터 값 자체가 이상한가"는 알려주지 않습니다. [회귀 테스트 편](dora-rs-av-regression-testing/#10-rerun으로-실패-시나리오-분석)에서 실패 시나리오 분석 도구로 다뤘던 **Rerun**은 회귀 테스트 상황에만 국한되지 않고, 개발 중 파이프라인 디버깅에도 그대로 씁니다. 노드의 처리 로직 코드는 그대로 두고, 중간에 "지금 처리 중인 프레임 이미지와 그 평균값을 Rerun으로도 남긴다"는 로깅 호출만 몇 줄 끼워 넣으면 됩니다.

이렇게 하면 실행 중인 프레임 이미지와 그 평균값의 시간에 따른 그래프를 Rerun 뷰어에서 동시에 볼 수 있습니다. `dora logs`가 "무슨 일이 일어났다"는 텍스트라면, Rerun은 "그 값이 실제로 이랬다"는 그림입니다 — 예를 들어 밝기 평균이 항상 0에 가깝게 나온다면, 로그에는 에러가 안 남지만 Rerun의 이미지 뷰를 보는 순간 프레임 자체가 새까맣다는 게(카메라 소스 로직 버그) 바로 드러납니다.
