---
title: "자율주행 카메라 모델 입문: 핀홀, 피쉬아이, FTheta"
...
피쉬아이 등거리 모델($r = f\theta$)은 단순하고 범용적입니다. FTheta는 이 단순한 선형 관계 대신 **$\theta$에 대한 다항식**으로 렌즈를 더 정밀하게 모델링합니다.

$$r(\theta) = a_1\theta + a_2\theta^3 + a_3\theta^5 + \cdots$$

- 짝수 차수 항은 대칭 왜곡에서 0이 되므로 홀수 차수만 씁니다.
- 계수 $a_1, a_2, a_3, \ldots$는 캘리브레이션으로 측정합니다.
- 피쉬아이 등거리 모델은 $a_1 = f$, 나머지 0인 FTheta의 특수한 경우입니다.

등거리 모델이 "직선 근사"라면 FTheta는 "곡선 근사"입니다. 가장자리까지 정확도가 필요할 때 FTheta가 유리합니다.

### 왜 자율주행에서 쓰이나

NVIDIA Hyperion 플랫폼(DriveAGX, Orin)과 NCore(NuRec) 파이프라인은 주변 시야 카메라로 120도 이상의 광각 렌즈를 씁니다. 이 렌즈들은 가장자리 왜곡이 커서 단순 등거리 모델로는 픽셀 오차가 수 픽셀 이상 남습니다. FTheta 다항식 모델로 캘리브레이션하면 전 화각에 걸쳐 오차를 0.5픽셀 이하로 줄일 수 있습니다.

**세 모델 비교**:

| 모델 | 투영 수식 | 화각 한계 | 주요 용도 |
|------|----------|---------|---------|
| 핀홀 | $r = f\tan\theta$ | ~90° | 협각·일반 카메라 |
| 피쉬아이 (등거리) | $r = f\theta$ | 180°+ | 서라운드뷰, 단순 광각 |
| FTheta | $r = \sum a_i\theta^{2i-1}$ | 180°+ | 고정밀 광각, ADAS/OEM |

### 주의: 모델 간 변환 불가

핀홀 파라미터에서 FTheta 파라미터를 수학적으로 유도하는 공식은 없습니다. 핀홀 캘리브레이션으로 얻은 $K$와 $[k_1, k_2, p_1, p_2]$를 FTheta 계수로 변환할 수 없다는 뜻입니다.

실무 영향: nuScenes와 AV2는 핀홀 카메라 데이터입니다. NCore(NuRec)는 FTheta를 요구합니다. 이 두 파이프라인을 연결하려면 같은 카메라를 두 모델로 각각 캘리브레이션하거나, 광각 렌즈 없이 핀홀 카메라만 쓰는 수밖에 없습니다. py123d에서 `PinholeCameraMetadata`와 `FThetaCameraMetadata`가 별도 타입으로 존재하는 이유입니다.
...
| **FTheta** | $r = \sum a_i\theta^{2i-1}$ — 고정밀 광각 모델. 계수끼리 수학적 변환 불가 |
...
*파이프라인 실무: py123d PinholeCameraMetadata, SE3 외부 파라미터, 카메라-LiDAR 합성, FTheta vs Pinhole 변환 불가 이슈는 [카메라 캘리브레이션: 내부 파라미터와 외부 파라미터](/docs/autonomous/data-infra/camera-calibration/)에서 다룹니다.*
