...
| **접선 왜곡 (Tangential)** | 이미지가 한쪽으로 기울어짐 | 렌즈와 이미지 센서가 완전히 평행하지 않아 생기는 조립 오차 |

#### 방사 왜곡이 생기는 이유

이상적인 핀홀 모델은 모든 입사각에서 $r = f\tan\theta$가 정확히 성립한다고 가정합니다. 그런데 실제 렌즈는 제조 방식과 광학 설계에 따라 가장자리로 갈수록 이 관계에서 벗어납니다. 이 **반경 방향(radial) 편차**가 방사 왜곡입니다.

비구면(aspherical) 고급 렌즈도 방사 왜곡은 존재합니다. 왜곡의 크기를 줄일 수 있을 뿐, 완전히 없앨 수는 없습니다.

왜곡 계수 중 가장 큰 영향을 주는 $k_1$의 부호에 따라 두 가지로 나뉩니다.

$$x' = x(1 + k_1 r^2 + k_2 r^4 + k_3 r^6) + 2p_1 xy + p_2(r^2 + 2x^2)$$

$$y' = y(1 + k_1 r^2 + k_2 r^4 + k_3 r^6) + p_1(r^2 + 2y^2) + 2p_2 xy$$

여기서 $(x, y)$는 **정규화 좌표**로, 카메라 좌표계의 3D 점을 $Z$로 나눈 값입니다: $(x, y) = (X/Z, Y/Z)$. 픽셀 단위가 아닌 무차원 좌표이며, $K$를 곱하기 전 단계의 좌표입니다.

- $r^2 = x^2 + y^2$: 정규화 좌표에서 주점까지의 거리 제곱
- $k_1, k_2, k_3$: 방사 왜곡 계수
- $p_1, p_2$: 접선 왜곡 계수 (조립 오차로 인한 미세한 기울어짐을 보정)

실제 픽셀 좌표는 왜곡 적용 후 $K$를 곱해서 구합니다: $u' = f_x x' + c_x$. **왜곡 보정(undistort)**은 이 과정의 역함수로, OpenCV가 수치적으로 처리해줍니다.

| 종류 | 모양 | 발생 상황 | $k_1$ 부호 |
|---|---|---|---|
| **배럴(Barrel) 왜곡** | 이미지가 바깥으로 부풀음 | 가장자리 굴절이 너무 강함. 광각렌즈에 흔함 | 음수 |
| **핀쿠션(Pincushion) 왜곡** | 이미지가 안쪽으로 당겨짐 | 가장자리 굴절이 너무 약함. 망원렌즈에 흔함 | 양수 |

자율주행 전방 카메라(일반 화각)는 주로 배럴 왜곡이, 피쉬아이는 배럴 왜곡이 극단적으로 심한 경우입니다.
...
핀홀 모델에서 입사각 $\theta$가 90도에 가까워지면 $\tan(\theta) \to \infty$가 되어 수식이 발산합니다. 화각이 넓어질수록 가장자리 투영 오차도 커지기 때문에, 핀홀 모델은 일반적으로 **화각 90도 이하**에서 안정적으로 동작합니다.
...
> **주의**: `cv2.findChessboardCorners`는 피쉬아이의 심한 왜곡 환경에서 코너 검출이 실패하는 경우가 많습니다. 실무에서는 `cv2.findChessboardCornersSB` (더 강인한 버전) 또는 ChArUco 보드를 사용하는 것을 권장합니다.
