...
## 5. 왜 Arrow가 아니라 Zarr인가, 왜 프레임 동기화가 없는가

이 시리즈의 [py123d](../py123d-for-beginners/)나 [Apache Arrow](../apache-arrow-internals/) 글에 익숙하다면, NCore가 Arrow의 제로카피·컬럼 기반 방식을 쓰지 않고, 센서별로 타임스탬프가 제각각인 채로 저장한다는 점이 낯설 수 있습니다. 이건 설계가 부실해서가 아니라, **NCore가 py123d와 완전히 다른 문제를 풀기 때문**입니다.

### Arrow는 "핫 패스에서 반복해서 빠르게 읽는" 포맷

py123d가 Arrow를 쓰는 이유는 ML 학습 중에 배치 하나를 몇 밀리초 안에 뽑아내야 하기 때문입니다. Arrow는 압축을 풀 필요 없이 메모리에 그대로 매핑해서 읽는 제로카피 포맷이라, 이런 반복적인 읽기에 최적화되어 있습니다.

### NCore/Zarr는 "한 번 통째로 읽어 긴 최적화를 돌리는" 포맷

반면 NCore가 먹이는 대상(NuRec)은 학습 배치가 아니라 [Gaussian Splatting 재구성](../nurec-neural-reconstruction-for-beginners/)입니다 — 시퀀스 하나를 통째로 불러다 놓고 몇 분~몇 시간짜리 최적화를 돌리는 워크로드입니다. 이 상황에서는 "초당 배치 몇 개"보다 다음 두 가지가 훨씬 중요합니다.

- **압축**: 카메라 JPEG 스트림, 라이다 리턴 값 같은 원본 로그는 용량이 어마어마합니다. Zarr는 청크 단위로 압축해서 저장하므로, 대규모 플릿 데이터를 디스크/네트워크에 올릴 때 용량을 크게 줄입니다. 반대로 Arrow의 제로카피는 압축을 안 하는 게 전제라, 대용량 원본 저장에는 오히려 안 맞습니다.
- **컴포넌트별 독립 버전 관리**: 2절에서 다룬 대로 포즈·카메라·라벨을 독립적으로 갱신할 수 있어야 합니다. 이건 "빠른 순차 읽기"보다 "부분만 골라 읽고 부분만 갱신하기"에 가까운 요구사항이고, Zarr의 청크 구조가 여기 맞습니다.

즉 Arrow는 핫 패스 반복 읽기에, Zarr는 압축된 대용량 원본을 저장하고 필요한 조각만 골라 읽는 데 최적화되어 있습니다 — 목적이 다르니 포맷도 다른 겁니다.

### 프레임 동기화가 없는 것도 같은 이유: 동기화는 저장이 아니라 읽는 쪽의 책임

카메라(30Hz)·라이다(10Hz)·IMU(수백Hz)는 애초에 물리적으로 동기화되어 있지 않습니다. 저장 단계에서 억지로 프레임을 맞추려면 리샘플링·보간이 필요한데, 그러면 원본 타이밍 정보가 손실됩니다.

그래서 NCore는 각 컴포넌트가 자기 고유의 타임스탬프를 그대로 갖게 하고, 동기화는 **읽는 쪽(소비자)의 책임**으로 미룹니다. 실제로 [py123d → NuRec](../py123d-to-nurec/)에서 쓰는 `NCoreParser`도 `iter_modalities_sync()`라는, NCore 바깥에서 동기화를 처리하는 별도 레이어를 씁니다 — NCore 저장 포맷 자체는 동기화를 신경 쓰지 않고, 그 책임을 py123d 같은 상위 레이어에 넘긴 것입니다.

---

## 6. 센서 모델: "이 카메라가 정확히 어떻게 세상을 투영하는가"
...
- **라이다 모델 평가 도구**로 6절의 라이다 센서 모델이 실제 데이터에 얼마나 잘 맞는지 검증할 수 있습니다.
...
### 10.1 재구성 자체에 수학적으로 반드시 필요한 3가지
...
- **캘리브레이션(`IntrinsicsComponent`)**: 6절에서 다룬 카메라 모델 파라미터입니다 — "이 픽셀이 실제로 어느 방향의 광선에 대응하는가"입니다. 이게 틀리면 가우시안이 엉뚱한 위치에 배치됩니다.

이 셋 중 하나라도 없으면 재구성 자체가 성립하지 않습니다. 나머지 컴포넌트는 전부 이 셋을 "보정"하거나 "품질을 높이는" 역할입니다.

### 10.2 AV 도메인에서 사실상 필수인 2가지
...
### 10.4 이 데이터, 전부 사람이 직접 만들어야 하나?

여기서 헷갈리기 쉬운 지점이 있습니다 — 위에서 "필요하다"고 한 컴포넌트가 전부 **원본 센서 로그**여야 하는 건 아닙니다. 실제로 3절의 컴포넌트 설명을 다시 보면, `PointCloudsComponent`는 "**Pre-computed** point clouds with typed attributes"라고 되어 있습니다 — 이미 사전 계산을 거친 파생 데이터라는 뜻입니다. 공식 문서는 이 컴포넌트의 예시로 "**SfM reconstructions, dense MVS outputs**"(구조-운동 복원, 멀티뷰 스테레오 결과물)를 직접 듭니다 — 즉 라이다보다는 오히려 **카메라 여러 장으로 만든 재구성 결과**에 가까운 예시입니다. 저장 방식도 PLY·LAS/LAZ·PCD 같은 외부 포인트 클라우드 파일 포맷을 감싸는 게 아니라, `xyz [N,3] float32`와 속성별 배열을 Zarr 안에 그대로 저장하는 방식입니다.
...
- **다른 모델·도구를 거쳐 나온 파생 데이터를 담는 컴포넌트**: 큐보이드(3D 객체 검출/트래킹 모델의 출력), 카메라 라벨의 깊이·세그멘테이션(별도의 깊이 추정·세그멘테이션 네트워크 출력), `PointCloudsComponent`(SfM/Dense MVS 등으로 미리 계산된 포인트 클라우드)처럼, **변환 전에 다른 파이프라인 단계를 한 번 거쳐야 나오는 값**입니다.
