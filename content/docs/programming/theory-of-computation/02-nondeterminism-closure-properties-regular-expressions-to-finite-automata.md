...
이 편에서 증명한 NFA-DFA 동등성과 정규 표현식→NFA 변환은 순수한 이론으로 끝나지 않습니다. [컴파일러(MIT 6.035) 시리즈의 1편: 어휘 분석](../compilers/01-lexical-analysis/)에서 정확히 이 두 정리가 렉서 생성기의 핵심 알고리즘(Thompson 구성, 부분집합 구성)으로 그대로 쓰이는 걸 볼 수 있습니다 — 그 글은 이 편의 증명을 다시 하지 않고, "정규 언어는 NFA로 표현 가능하고 NFA는 DFA로 변환 가능하다"는 사실을 공학적으로 어떻게 재사용하는지에 집중합니다.
