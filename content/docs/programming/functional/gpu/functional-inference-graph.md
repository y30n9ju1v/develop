...
이전 글 [액션/계산/데이터](/docs/programming/functional/basics/functional-actions-calculations-data/), [함수형 렌더 그래프](/docs/programming/functional/gpu/functional-render-graph/), [GPU Compute 셰이더를 함수형으로](/docs/programming/functional/gpu/functional-gpu-compute/)를 먼저 읽으면 더 자연스럽게 이어집니다.

---

## DNN 추론이 렌더 그래프와 닮은 이유

[함수형 렌더 그래프](/docs/programming/functional/gpu/functional-render-graph/) 글에서 렌더 패스를 **선언(데이터) → 의존 분석(계산) → GPU 실행(액션)** 세 단계로 나눴습니다. DNN 추론 파이프라인도 구조가 같습니다.
...
`run_detection`은 백엔드 트레잇만 받습니다. `OrtBackend`인지 `CpuRefBackend`인지 모릅니다. [함수형 DI](/docs/programming/functional/basics/functional-dependency-injection/) 패턴입니다.
...
*관련 글: [액션, 계산, 데이터](/docs/programming/functional/basics/functional-actions-calculations-data/), [함수형 렌더 그래프](/docs/programming/functional/gpu/functional-render-graph/), [GPU Compute 셰이더를 함수형으로](/docs/programming/functional/gpu/functional-gpu-compute/), [함수형 DI](/docs/programming/functional/basics/functional-dependency-injection/), [함수형 포인트 클라우드 처리](/docs/programming/functional/autonomous/functional-point-cloud/)*
