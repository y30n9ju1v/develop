...
### `IO.Ref` 대신 `StateM`을 썼다면

Lean에서 가변 상태를 표현하는 방법이 `IO.Ref`만 있는 건 아닙니다. `StateM σ α` (내부적으로 `σ → (α × σ)`, "상태를 하나 받아서 결과와 새 상태를 함께 돌려주는 순수 함수")를 쓰면, `IO`로 전혀 오염시키지 않고도 은행 계좌의 `balance`를 흉내 낼 수 있습니다.

```lean
def withdrawState (amount : Int) : StateM Int (Except String Int) := do
  let bal ← get
  if bal >= amount then
    set (bal - amount)
    return .ok (bal - amount)
...
#eval (withdrawState 50).run 100
-- (Except.ok 50, 50)
```

`(withdrawState 50).run 100`이 하는 일을 뜯어보면, `StateM`이 실제로는 아무것도 "변이"시키지 않는다는 게 드러납니다 — `run`은 그냥 `Int → (Except String Int × Int)` 모양의 순수 함수 하나를 호출한 것이고, 들어간 `100`이라는 값 자체는 여전히 불변입니다. 겉으로는 `IO.Ref`처럼 "상태를 갖고 순서대로 갱신되는" 것처럼 `do` 표기법으로 읽히지만, `bind`가 하는 일은 그저 이전 함수가 반환한 새 상태 값을 다음 함수의 입력으로 그대로 넘겨주는 것뿐입니다. 이런 의미에서 `StateM`은 여전히 순수하고 참조 투명합니다 — 같은 초기 상태로 `run`을 두 번 호출하면 항상 같은 결과가 나옵니다.

그런데 이게 바로 `StateM`이 이 장의 `withdraw`를 다시 짜는 데 정직한 선택이 **아닌** 이유이기도 합니다. SICP가 3.1절에서 보여주려는 건 정확히 "계좌마다 자기만의, 다른 계좌와 완전히 독립된 숨겨진 정체성(identity)을 갖는다"는 것입니다 — `acc1`과 `acc2`는 각자 자기 잔액을 스스로 들고 다니고, 서로의 존재를 모릅니다. `IO.Ref`는 이걸 그대로 모델링합니다: `IO.mkRef`를 두 번 호출하면 진짜로 서로 다른 두 개의 가변 셀이 생기고, 그 둘을 같은 함수에 넘겨도 프로그램 텍스트 어디에도 "지금 상태가 몇 번째로 스레딩되고 있는지" 같은 배관(plumbing) 코드가 필요 없습니다.

`StateM`으로 계좌를 표현하려면, 그 계좌의 상태(`Int`)를 **프로그램 전체에 걸쳐 명시적으로 실을 꿰어 전달**해야 합니다 — 계좌가 여러 개면 상태 타입도 `(Int × Int)`처럼 계좌 개수만큼 커지고, 어느 함수가 어느 계좌의 상태를 어떤 순서로 읽고 쓰는지를 타입 시그니처가 전부 정직하게 드러내야 합니다. 이건 장점이자 단점입니다 — **장점**은 `IO.Ref`가 숨기는 것(정확히 몇 개의 가변 셀이 존재하고 누가 무엇을 공유하는지)을 타입이 전부 드러내 준다는 것이고, **단점**은 SICP가 3.1절에서 강조하는 "객체가 자기만의 숨겨진 상태를 갖고, 서로 독립적으로 시간에 따라 변한다"는 정확히 그 모델을 표현하기가 훨씬 번거로워진다는 것입니다. 요약하면: `IO.Ref`는 **물리적 가변성**(진짜로 존재하는, 정체성이 있는 셀)을 모델링하고, `StateM`은 **논리적 가변성**(참조 투명성을 유지한 채 상태를 실로 꿰어 전달하는 것)을 모델링합니다 — SICP 3.1절의 논지는 전자를 요구하므로, 이 장에서는 `IO.Ref`가 번역으로서 더 정직한 선택입니다.
...
| 물리적 가변성 vs 논리적 가변성 | 항상 물리적(진짜 셀) | `IO.Ref`(물리적, 정체성 있음) vs `StateM`(논리적, 상태를 실로 꿰어 전달) |
