...
> ```
>
> 4가지 중 실제로 의미 있는 해는 하나입니다 — 3D 점이 두 카메라 **앞**에 있어야 하기 때문입니다. 카메라 뒤에 3D 점이 복원되는 경우는 물리적으로 불가능하므로 나머지 3개는 자동으로 제거됩니다. 이 과정에서 유효하지 않은 대응점을 걸러내는 `mask_pose`를 반환하므로, 삼각측량 시 이 마스크를 반드시 적용해야 합니다.

> **💡 핵심 개념: Scale Ambiguity (스케일 모호성)**
> 사진만으로는 우리가 찍은 것이 "실제 10m 크기의 커다란 집"인지, 아니면 "10cm 크기의 정교한 장난감 집"인지 알 방법이 없습니다.
> 따라서 수학적으로 복원된 카메라의 이동 거리 `t` 역시 "오른쪽으로 조금 이동했다"는 방향(Direction) 정보만 정확할 뿐, "실제로 몇 미터(m)를 이동했는지"에 대한 실제 크기(Scale) 정보는 소실되어 있습니다. (이것을 단안 카메라의 스케일 모호성이라고 부릅니다.)

**주의:** 위 이유 때문에 복원된 `t`는 방향만 맞고 크기(scale)가 없습니다. 이 문제는 4편에서 실제 거리 값을 가진 LiDAR 센서 데이터를 융합하여 해결하게 됩니다.
