...
- **[이론을 C++23으로 옮기다: 안전한 병렬 프로그래밍](safe-parallel-cpp23/)** — 왜 안전하지 않은가(1부) → 타입 상태로 세션 타입 흉내내기(2부) → 이동 전용 타입으로 선형성 흉내내기(3부) → 범주론적 인터페이스(4부) → 동시성 분리 논리(5부)까지, linear-and-session-types와 범주론 책들의 이론을 타입 상태 패턴·이동 전용 타입·std::span 분할·concepts로 실제 C++23(Apple Clang 21) 코드로 인코딩해 컴파일 타임/TSan·ASan 검증으로 병렬 버그를 잡고, 맺음말에서 Rust가 이 이론들 중 무엇을 언어 코어로 채용했는지(RustBelt의 형식 증명까지)까지 정리하는 독립적인 실무 입문서
