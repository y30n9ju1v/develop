...
- **[Mathlib으로 수학하기](lean4-and-proofs/mathlib-mathematics/)** — 증명 기본기(1부) 위에 집합과 함수(2부), 정수론(3부), 군과 순서 구조(4부), 수열의 극한과 연속함수(5부)를 Mathlib으로 직접 증명하며 다지는 독립적인 Lean4 입문서
...
- **[람다 계산법과 계산 가능성](category-theory-and-fp/lambda-calculus-and-computability/)** — 함수를 만드는 것과 함수에 넣어보는 것, 이 두 규칙만으로 숫자·참거짓·이름 없는 재귀(Y 콤비네이터)까지 만들어내고(1~2부), 튜링 기계라는 다른 계산 모델과 나란히 놓아 둘이 같은 힘을 가진다는 것(처치-튜링 명제)과 대각선 논법으로 어떤 방법으로도 풀 수 없는 문제(정지 문제)가 존재한다는 것까지 증명한 뒤(3부), 이 이론이 함수형 프로그래밍과 괴델의 불완전성 정리에 남긴 흔적을 짚는(4부) 독립적인 입문서. 수식보다 논증으로 읽어가도록 쓰였고, 부록에서 처치 수 2+3이 정말 5로 계산되고 이름 없는 재귀가 실제로 동작한다는 걸 파이썬으로 직접 실행해 확인한다
- **[함수형 사고로 알고리즘 다시 보기](category-theory-and-fp/functional-thinking-for-algorithms/)** — 재귀를 반복문의 특수한 변형이 아니라 정의를 정의로 푸는 방법으로 다시 소개하고, 알고리즘이 왜 항상 맞는지를 신뢰가 아니라 구조적 귀납으로 확인하는 법을 배우는 책. 병합·퀵 정렬을 귀납과 불변식으로 증명하고(2부), 리스트·트리·이진 탐색 트리를 재귀적 자료구조로 다시 짓고(3부), 순진한 피보나치의 지수적 폭발을 메모이제이션과 타뷸레이션으로 무너뜨리고(4부), 그래프 탐색과 다익스트라의 그리디 선택을 귀납으로 정당화한 뒤(5부), 꼬리 재귀를 반복문으로 바꾸는 기계적 변환과 「람다 계산법과 계산 가능성」의 Y 콤비네이터로 돌아가 재귀가 왜 애초에 가능했는지 되짚으며 마무리한다(6부). 모든 코드는 실제로 실행해 나온 결과를 그대로 싣는다
- **[P vs NP](category-theory-and-fp/p-vs-np/)** — 스도쿠는 풀기는 어려운데 확인은 쉽다는 관찰에서 출발해, 다항 시간과 지수 시간(1부), 확인이 쉬운 문제들의 모임 NP와 P vs NP라는 핵심 질문(2부), 환원과 NP-완전이 왜 수천 개의 문제를 하나로 묶는지 그리고 왜 반세기 동안 미해결인지(3부), 암호학·최적화에서의 의미와 알려진 것/모르는 것의 경계(4부)까지 다루는 독립적인 입문서. 부록에서 부분집합 합 문제를 브루트포스로 직접 돌려, n이 16에서 24로 늘어나는 동안 걸리는 시간이 35ms에서 11798ms로 폭증하는 반면 답 확인은 0.01ms에 그친다는 걸 실측으로 확인한다
- **[정보 이론 (섀넌)](category-theory-and-fp/shannon-information-theory/)** — 동전 던지기와 해가 떴다는 소식 중 어느 쪽이 더 많은 정보를 담고 있는가라는 질문에서 출발해, 확률로 놀라움의 크기를 재는 법과 엔트로피(1부), 압축에는 엔트로피라는 절대 한계가 있다는 섀넌의 소스 코딩 정리(2부), 노이즈 속에서도 오류율을 얼마든지 낮출 수 있다는 채널 용량과 오류 정정 부호(3부), 오늘날 디지털 세상·열역학과의 접점·이 이론이 의도적으로 다루지 않은 "의미"까지(4부) 다루는 독립적인 입문서. 부록에서 반복 텍스트·자연어·무작위 텍스트 각각의 엔트로피를 계산해 실제 gzip 압축률과 비교하고, 무작위 텍스트에서는 이론적 한계와 거의 정확히 일치하는 반면 구조가 있는 텍스트에서는 gzip이 단순한 문자 빈도 추정치보다 훨씬 더 압축한다는(문자 간 상관관계까지 반영해야 진짜 엔트로피에 가까워진다는) 걸 실측으로 확인한다
- **[괴델의 불완전성 정리](category-theory-and-fp/godels-incompleteness-theorems/)** — 힐베르트의 꿈(수학 전체를 공리와 규칙만으로 완벽하게 정리하는 것)이 왜 무너졌는지를 다루는 독립적인 입문서. 형식 체계·완전성·무모순성(1부)에서 시작해, 우편번호처럼 문장 자체를 숫자로 부호화하는 괴델 수 매기기와 증명 가능성의 산술적 표현(2부), 대각선 보조정리로 "나는 증명될 수 없다"는 문장 G를 실제로 지어 제1불완전성 정리를 완성하는 과정(3부), 체계가 자신의 무모순성조차 증명할 수 없다는 제2불완전성 정리와 튜링의 정지 문제와의 접점(4부)까지 다룬다. 부록에서 자기 자신을 그대로 출력하는 파이썬 쿼인을 실제로 작성·실행해, 출력을 원본과 diff로 비교하고 그 출력을 다시 실행해도 똑같다는 것까지 확인해 자기 지시가 실제로 구현 가능한 고정점이라는 걸 검증한다
...
- **[RIIR: C의 JSON 파서를 Rust로 다시 쓰고 Python에 배포하기](systems-and-parallel-programming/riir-json-parser/)** — cJSON을 기준 삼아 JSON 파서를 Rust로 지으며 소유권·라이프타임·안전한 병렬처리를 실제 컴파일과 C 비교로 익히고(1~5부), PyO3와 maturin으로 Python 확장 모듈을 만든 뒤 Result를 예외로 매핑하고 표준 json과 정확성을 비교하며(6~7부), PyObject 변환 비용·GIL·병렬성을 실측으로 분석하고(8부), wheel과 abi3로 배포 가능한 패키지까지 완성하는(9부) 실무 입문서
