...
- **[증명보다 계산이 먼저다: 공업수학을 코드로 다시 배우기](engineering-math-in-code/)** — 상미분방정식(1부) → 라플라스 변환(2부) → 선형대수(3부) → 벡터 미적분(4부) → 푸리에 해석과 편미분방정식(5부)까지, 정의-정리-증명 순서 대신 "왜 필요한가 → NumPy/SciPy/SymPy로 계산해서 확인 → 수식 읽기" 순서로 공업수학 표준 커리큘럼을 다시 짓는 독립적인 입문서. 매 장마다 손으로 유도한 해석해와 서로 다른 방법으로 얻은 수치해를 교차 검증한다
