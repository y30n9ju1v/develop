...
.초보자 이정표
====
*선수지식:* Python 함수의 매개변수와 반환값. 타입 힌트는 처음이어도 됩니다. 예제는 Python 3.11 이상을 사용합니다.

*이번 장의 도착점:* 타입 힌트가 문서 이상의 계약이 되어, ``None``·실패·잘못된 상태를 실행 전에 발견하도록 돕는다는 점을 봅니다.

*핵심 낱말:* 타입 힌트(type hint), 정적 검사(static checking), ``Optional``, ``Result``, 좁히기(narrowing). mypy 설정과 고급 제네릭은 처음 읽기에서 건너뛰어도 됩니다.
====
