...
categories: ["books"]
description: "Applicative 관점으로 독립적인 작업을 동시 실행하고 결과를 조합하는 원리 — asyncio.gather의 철학과 한계를 다룹니다."
---
...
.초보자 이정표
====
*선수지식:* 2장의 ``gather``와 4장의 순차 의존성. Applicative라는 이름은 몰라도 됩니다.

*이번 장의 도착점:* 서로 결과에 의존하지 않는 작업을 함께 시작하고, 모두 끝난 결과를 한 번에 조합하는 경우를 순차 흐름과 구분합니다.

*핵심 낱말:* 독립성(independence), ``gather``, Applicative, 동시성, 병렬성. 취소·예외의 세부 규칙은 첫 읽기에서 NOTE만 기억하고 뒤에서 다시 보아도 됩니다.
====
...
하지만 실무에서는 서로 *독립적인* 여러 작업을 함께 진행하고 싶을 때가 많습니다. 다섯 명의 사용자 정보를 가져올 때, 한 명씩 순차적으로 조회할 이유가 없습니다. 각 조회는 완전히 독립적이므로 함께 시작할 수 있고, 그렇게 하면 I/O 대기 시간을 겹칠 수 있습니다. 이 장에서는 독립적인 비동기 작업을 **동시성(concurrency)**으로 실행하고 결과를 조합하는 ``Applicative`` 관점을 배울 것입니다.

[NOTE]
====
이 장의 ``asyncio.gather`` 예제는 단일 이벤트 루프에서 I/O 대기를 번갈아 처리하는 *동시성*입니다. 여러 CPU 코어에서 Python 바이트코드를 동시에 실행하는 *병렬성*은 아닙니다. CPU 중심 작업에는 ``asyncio.to_thread()``, 프로세스 풀 또는 별도 워커 같은 다른 전략이 필요합니다. 또한 ``gather``는 Monad/Applicative의 직관을 설명하는 좋은 비유이지만, 취소·예외·즉시 실행 같은 관찰 가능한 효과 때문에 수학적 Applicative 법칙을 보장하는 타입 그 자체라고 단정할 수는 없습니다.
====
...
- *결론*: 의존적인 ``await`` 흐름은 flatMap/bind와 대응
...
*6장: 동시성과 병렬성 - 독립적 작업의 동시성 합성*

link:../04-monad/[4장]의 flatMap은 **순차 합성**이었습니다. "A가 끝나면 B 시작"의 패턴이죠. 하지만 A와 B가 서로 독립적이라면? 함께 시작해 I/O 대기를 겹칠 수 있습니다. 이 장에서는 *독립 작업의 동시성 합성* — 여러 작업을 함께 실행하고 결과를 조합하는 방법을 배웁니다. 이것이 **Applicative**가 제공하는 핵심 직관입니다.
...
== asyncio.gather: 동시 실행의 시작

파이썬의 asyncio는 여러 코루틴을 동시에 실행하는 간단한 방법을 제공합니다. asyncio.gather입니다.

[source,python]
----
async def get_weather_dashboard_concurrently(city: str) -> dict:
    """날씨 대시보드 데이터를 동시성으로 가져옵니다"""
    print(f"=== 동시 실행 시작: {city} ===")
    start = time.time()
...
# 실행
result = asyncio.run(get_weather_dashboard_concurrently("서울"))
----
...
세 작업이 거의 동시에 시작되고, 거의 동시에 완료됩니다. 총 소요 시간은 1초입니다. 순차 실행의 3초에 비해 약 3배 빠릅니다. 이것은 I/O 대기를 겹치는 동시성의 힘입니다.

asyncio.gather가 하는 일을 정확히 이해해봅시다. gather는 여러 awaitable을 받고, 전달된 코루틴은 Task로 스케줄링합니다. 모두 성공하면 완료될 때까지 기다렸다가, 결과를 *입력 순서대로 담은 리스트*로 반환합니다. 완료 순서와는 관계없습니다. 기본값에서는 하나가 예외를 내면 그 예외를 즉시 전파하지만, 나머지 작업은 자동 취소되지 않고 계속 실행된다는 점도 반드시 기억하세요.
...
다섯 개 도시를 순차적으로 조회하면 5초가 걸리지만, I/O 대기를 동시성으로 겹치면 1초 정도에 끝납니다. 작업이 많을수록 동시 실행의 이점은 더욱 커집니다.
...
세 가지 의존성 중 하나라도 있으면 순차 실행이 필요하고, 모두 없으면 동시 실행을 고려할 수 있습니다.
...
만약 모든 것을 순차적으로 실행했다면 3초가 걸렸을 것입니다. 하지만 I/O 대기를 동시 실행으로 겹쳐 2초로 줄였습니다. 이것은 작지만 의미 있는 최적화입니다. 사용자 경험 측면에서 1초의 차이는 크게 느껴질 수 있습니다.
...
# Applicative (gather): 독립 작업을 동시 실행 가능
...
flatMap은 "이전 결과를 사용해서 다음 작업 결정"이라는 의미를 가지므로 순차적인 의존 관계를 표현합니다. 반면 Applicative의 ap는 **"독립적인 작업들의 결합"**을 표현합니다. 이 "독립성"이 성능 최적화의 열쇠입니다. 작업들 사이에 의존성이 없다면, I/O 작업을 동시에 시작해 대기 시간을 절약할 수 있습니다.
...
모든 개념을 종합해서 실용적인 동시성 크롤러를 만들어봅시다. ``Semaphore``로 동시 접속 수를 제어하고, ``gather``로 동시 실행하며, 결과와 에러를 수집합니다.
...
만약 순차적으로 실행했다면 ``delay/2`` 때문에 최소 2초 이상 걸리고, 타임아웃까지 더해져 훨씬 느렸을 것입니다. I/O 동시 실행 덕분에 전체 시간은 가장 느린 요청(2초)과 비슷하게 걸립니다.

== 심화: 동시 실행의 실전 주의점

동시 실행은 I/O 처리량을 높이지만, 잘못 사용하면 오히려 시스템을 마비시킬 수 있습니다. 실무에서 마주치는 함정들을 살펴봅시다.
...
동시 실행은 **강력하지만 위험**합니다. link:../01-functional-thinking/[1장]에서 배운 참조 투명성과 부수 효과 제어가, 동시성 세계에서는 **동시성 제어**와 **자원 관리**로 이어집니다. 함수형 프로그래밍의 철학 - 예측 가능성, 합성 가능성 - 은 여기서도 여전히 유효합니다.
...
이번 장에서는 link:../04-monad/[4장]의 순차 합성(Monad)을 넘어, **독립 작업의 동시성 결합(Applicative 관점)**으로 I/O 대기 시간을 줄이는 방법을 배웠습니다.

[arabic]
. *순차 vs 병렬의 기준*: "이 작업의 결과가 다음 작업에 필요한가?" (의존성 여부)

* 의존적임 → *Monad* (``await`` / ``flatMap``) → 순차 실행
* 독립적임 → *Applicative* (``gather``) → 동시 실행

. *도구들*:

* ``asyncio.gather``: 가장 기본적인 동시 실행 도구
...
|*6장* |Applicative (gather) |**독립적 작업**의 동시성 결합 |*비동기 동시성*
|===
