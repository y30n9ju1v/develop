...
.초보자 이정표
====
*선수지식:* 2장의 ``async``/``await``와 4장의 순차 합성. 이 장부터는 선택 심화입니다.

*이번 장의 도착점:* ``await``에서 중단한 함수가 어떻게 다시 이어지는지 직관적으로 보고, 효과를 값·타입·실행기로 분리하려는 Effect System의 문제의식을 이해합니다.

*핵심 낱말:* continuation, 효과(effect), 효과 시스템(effect system), 핸들러(handler). 구현 세부와 특정 라이브러리는 나중에 읽어도 되며, "무엇을 중단했다가 재개하는가"만 잡으면 충분합니다.
====
...
*왜 이것이 중요할까요?* async와 await는 CPS와 같은 "이후에 할 일"이라는 관점으로 이해할 수 있기 때문입니다. 다만 CPython이 async 함수를 문자 그대로 CPS 함수로 변환하는 것은 아닙니다. 네이티브 코루틴 프레임과 awaitable 프로토콜, 이벤트 루프의 콜백을 통해 중단 지점과 재개를 관리합니다. awaitable이 완료되면 이벤트 루프가 해당 Task를 다시 실행 가능하게 만들고, 코루틴은 멈춘 지점부터 계속됩니다.
...
=== contextvars는 Reader 효과와 닮은 실용적 도구입니다

link:../08-practical-patterns/[8장]에서 ``contextvars``로 요청 ID 같은 실행 문맥을 암묵적으로 전달하는 방법을 실용적인 관점에서 다뤘습니다. 이를 Reader 효과의 **직관에 비춰** 볼 수는 있습니다. 읽기 전용 환경을 함수 인자 없이 조회한다는 점이 닮았기 때문입니다.

8장의 ``ContextVar``를 다시 뜯어보면, ``request_id.set(...)``으로 현재 실행 문맥에 값을 넣고 ``request_id.get()``으로 읽습니다. Reader 효과가 모델링하는 "환경(Environment)을 읽는다"는 관심사와 유사합니다. 특히 asyncio Task가 문맥을 복사해 보존하므로 요청 ID·추적 정보처럼 작업에 귀속되는 값을 다룰 때 유용합니다.

하지만 ``contextvars`` 자체가 Algebraic Effect System은 아닙니다. 효과 연산의 대수적 인터페이스, 프로그램을 해석하는 핸들러, 효과별 처리 정책을 제공하지 않으며, 값 설정은 가변적인 동적 문맥을 바꿉니다. 따라서 여기서는 "Reader와 닮은 문맥 전달 도구"로 한정해 사용하고, 아래의 ``Program``/``EffectHandler`` 예제처럼 기술과 해석을 분리하는 모델과 구별해야 합니다.

이 구분을 지키면, 실용적 문맥 전달과 효과를 명시적으로 모델링하는 설계를 혼동하지 않고 각각의 장점을 활용할 수 있습니다.
...
- 제어 흐름 자체를 값으로 만듦
- "프로그램의 나머지"를 포착하고 조작
- *우리가 배운 것*: async/await의 중단·재개를 Continuation 관점으로 해석할 수 있음


*계층 2: Functor와 Monad* (link:../03-functor/[3장]-link:../04-monad/[4장])
...
- 여러 독립적인 계산을 조합
- *우리가 배운 것*: ``asyncio.gather``는 독립 작업의 결합을 Applicative 관점으로 읽게 해 줌
...
- *우리가 배운 것*: Program {plus} EffectHandler는 기술과 해석을 분리하는 교육용 모델; contextvars는 Reader와 닮은 문맥 전달 도구
...
이 그림은 학습을 위한 연결 지도이지, 엄격한 역사적·의존성 계층은 아닙니다. Continuation, Monad, Applicative, Algebraic Effects는 서로 다른 문제를 다루며 연구사에서도 독립적으로 발전하고 영향을 주고받았습니다.
...
=== dry-python/returns: 실전 함수형 컨테이너

Algebraic Effects와 IO Monad, ZIO 같은 효과 모델은 서로 구분해야 합니다. 파이썬 표준 라이브러리에는 Algebraic Effect System이 없지만, Result와 Maybe처럼 실패·부재를 명시하는 함수형 컨테이너를 제공하는 라이브러리는 있습니다.

**``dry-python/returns`` 라이브러리**는 Result, Maybe, IO 같은 함수형 타입을 제공합니다. 이를 Algebraic Effect System으로 부르기보다는, 파이썬 코드에서 명시적인 결과 타입과 합성 패턴을 연습할 수 있는 도구로 보는 편이 정확합니다.
...
# 외부 I/O의 예외를 Result로 변환
@safe
def read_config(file_path: str) -> dict:
    """설정 파일을 읽고, 예외는 Failure로 바꿉니다"""
...
def load_api_key(file_path: str) -> Result[str, str | Exception]:
    ...
    return flow(
        file_path,
        read_config,  # Result[dict, Exception]
...
3. *테스트 용이*: 경계의 ``read_config``와 순수한 검증 함수를 분리하여 테스트가 간단
...
3. *3단계*: 필요할 때 dry-python/returns 같은 컨테이너로 파이프라인 구축
4. *4단계*: 효과의 기술·해석 분리가 실제로 이득인 복잡한 도메인만 별도 모델 검토
...
- *파이썬과의 연관*: 표준 효과 시스템이 아니라, ``dry-python/returns`` 같은 라이브러리와 타입 힌팅으로 일부 패턴을 명시적으로 적용 가능
...
[arabic]
. *Continuation*: ``async/await``의 중단·재개는 Continuation 관점으로 이해할 수 있습니다. 네이티브 코루틴은 제너레이터와 관련된 별도 객체입니다.
. *Algebraic Effects*: 효과를 기술하고 핸들러가 해석하는 모델은 프로그램의 의미(What)와 실행(How)을 분리합니다. ``contextvars``는 Reader와 닮은 문맥 전달 도구이지 그 자체가 효과 시스템은 아닙니다.
. *실전*: ``dry-python/returns``는 명시적인 Result/Maybe/IO 컨테이너를 연습할 도구입니다. 팀의 필요가 확인된 작은 경계부터 도입하세요.
