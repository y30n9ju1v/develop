...
.초보자 이정표
====
*선수지식:* 4장의 순차 합성. Python 예외 처리를 조금 써 봤으면 충분합니다.

*이번 장의 도착점:* 실패를 예외로 멀리 던지지 않고 ``Result`` 같은 값으로 돌려, 성공과 실패 양쪽을 호출자가 코드에서 다루게 만듭니다.

*핵심 낱말:* 예외(exception), ``Result``, 성공(success), 실패(failure), 타입(type). 복잡한 타입 표기보다 먼저 ``Ok``와 ``Err``의 두 갈래를 따라가세요.
====
...
- *콜백은 합성 비용이 큽니다*: 콜백 규약과 제어 흐름이 호출자에게 전파됨
...
- *yield from → await*: 중단·재개 모델의 역사적 연결
...
=== asyncio.gather를 활용한 동시 실행

여러 Result를 반환하는 비동기 작업을 I/O 동시성으로 실행하고 싶을 때는 ``asyncio.gather``를 사용합니다.

[source,python]
----
async def read_files_concurrently(filenames: list[str]) -> Result[list[str], str]:
    """여러 파일을 동시성으로 읽고 결과를 합칩니다"""
...
이 장에서 우리는 Result와 Railway-Oriented Programming을 통해 **에러를 값으로 다루며 순차적으로 합성**하는 방법을 배웠습니다. ``read++_++file → parse++_++json → validate → save`` 파이프라인은 각 단계가 **의존적**이었습니다. 이전 단계의 결과가 있어야 다음 단계를 시작할 수 있었죠.
...
*핵심 질문*: ``asyncio.gather``는 어떻게 여러 비동기 작업을 동시 실행하고 결과를 조합할까요? 기본 예외 전파와 취소는 어떻게 처리될까요? 이것이 함수형 프로그래밍의 어떤 패턴을 떠올리게 할까요?

link:../06-concurrency-and-parallelism/[6장]에서는 이 질문에 답합니다. **Applicative**라는 관점으로 독립적인 작업들을 동시성으로 결합하는 방법을 배웁니다. ``flatMap``이 순차 합성의 도구였다면, Applicative는 독립 계산의 결합을 설명하는 도구입니다.

=== 다음 6장 예고: Applicative Functor (동시성)

다음 장에서는 **독립적인 작업들을 동시 실행**하고 결과를 조합하는 **Applicative 관점**을 다룹니다.
``asyncio.gather``를 통해 순차 합성을 넘어, 독립 작업 결합의 세계로 나아갑시다.
// end::body[]
