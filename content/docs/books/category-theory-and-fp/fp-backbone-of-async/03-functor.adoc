...
.초보자 이정표
====
*선수지식:* 1장의 순수 함수와 2장의 비동기 작업. Functor라는 말을 안다고 가정하지 않습니다.

*이번 장의 도착점:* 값이 바로 있든 ``Optional``·``Future`` 같은 컨텍스트 안에 있든, 값만 바꾸는 함수는 ``map``으로 적용할 수 있음을 봅니다.

*핵심 낱말:* 컨텍스트(context), ``map``, Functor, 항등·합성 법칙. 법칙의 이름보다 "상자를 열지 않고 안의 값만 바꾼다"는 그림을 먼저 기억하세요.
====
...
[WARNING]
.``map``이 삼켜도 되는 예외는 없습니다
====
초기 버전에서 ``func(self._value)``를 ``try``로 감싸 "함수 실행 중 아무 예외가 나든 조용히 ``Maybe(None)``으로 바꾼다"는 유혹을 느낄 수 있습니다. 이건 안티패턴입니다 — ``Maybe``가 표현하려는 건 "값이 있을 수도, 없을 수도 있다"는 **부재(absence)**이지, "계산이 실패할 수도 있다"는 **실패(failure)**가 아닙니다. 이 둘을 구분하지 않고 ``except Exception``으로 모든 예외를 흡수하면, 정상적인 부재("주소가 없는 사용자")와 진짜 버그(``addr.get`` 호출에서 오타로 난 ``TypeError``)가 똑같이 ``Maybe(None)``으로 뭉개져서 디버깅이 거의 불가능해집니다. 함수 실행이 실패할 수 있는 경우를 명시적으로 표현하고 싶다면, 그건 ``Maybe``가 아니라 link:../05-error-handling-as-types/[5장]에서 다룰 ``Result`` 타입의 역할입니다.
====
...
[cols=",,",options="header",]
|===
|특성 |콜백 (link:../02-why-async/[2장]) |Future {plus} map (*3장*)
...
def dict_map(func: Callable[[V], U], d: dict[K, V]) -> dict[K, U]:
    # 구현해보세요
    pass
----
...
[source,python]
----
def test_functor_laws(future: Future[T], f: Callable[[T], T], g: Callable[[T], T]):
...
[source,python]
----
def all_futures(futures: list[Future[T]]) -> Future[list[T]]:
    """
...
[source,python]
----
def get_user_latest_post_title(user_id: int) -> None:
...
# ✅ 이렇게 작동했으면 좋겠다
user_future: Future[User] = fetch_user(123)
post_future: Future[Post] = user_future.flat_map(fetch_latest_post)
#                                      ^^^^^^^^
#                                      중첩을 자동으로 풀어주는 연산자!
...
- async/await: 의존적인 비동기 단계를 선형적으로 적는 문법 (Monad의 ``bind``/do-notation과의 대응)
