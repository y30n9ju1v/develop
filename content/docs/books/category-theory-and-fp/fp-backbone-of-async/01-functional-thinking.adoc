...
.초보자 이정표
====
*선수지식:* Python 함수·조건문·리스트. 수학이나 범주론은 필요 없습니다.

*이번 장의 도착점:* 같은 입력에 같은 결과를 내는 작은 함수를 만들고, 값을 바꾸지 않고 새 값을 돌려주는 코드가 왜 예측하기 쉬운지 확인합니다.

*핵심 낱말:* 순수 함수(pure function), 불변성(immutability), 참조 투명성(referential transparency), 함수 합성(composition). Promise·Future·Observable의 역사적 연결은 처음에는 건너뛰어도 됩니다.
====
