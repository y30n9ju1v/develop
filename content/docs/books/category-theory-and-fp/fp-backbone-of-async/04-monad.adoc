...
categories: ["books"]
description: "Monad와 flatMap으로 의존적인 비동기 작업을 합성하는 원리, 그리고 async/await와의 정확한 대응을 다룹니다."
---
...
.초보자 이정표
====
*선수지식:* 3장의 ``map``과 2장의 ``await``. 모나드를 미리 알 필요는 없습니다.

*이번 장의 도착점:* 다음 단계가 평범한 값이 아니라 또 다른 비동기 작업을 만들 때, 중첩된 컨텍스트를 ``flatMap``으로 한 단계로 잇는 이유를 이해합니다.

*핵심 낱말:* 중첩(nesting), ``flatMap``, Monad, 순차 의존성. ``await``는 그 자체가 ``flatMap``은 아니며, 의존적인 ``async def`` 흐름 전체가 순차 합성과 닮았다는 점만 우선 잡으세요.
====
...
이 세 가지 능력이 결합되면, "기다리는 척하는" 코드를 작성할 수 있습니다. 제너레이터 → yield from → async/await의 계보는 중단 가능한 계산을 선형적으로 표현하게 했고, 의존적인 단계 연결은 flatMap/bind와 대응합니다.
...
파이썬 3.5에서 async와 await 키워드가 추가되었습니다. 이 문법은 제너레이터 기반 코루틴에서 얻은 중단·재개 모델을 더 명확하게 표현합니다. 네이티브 코루틴은 제너레이터와 비슷하게 중단될 수 있지만, 제너레이터 그 자체는 아닙니다. ``await``는 ``yield from``과 비슷하게 awaitable에 제어를 넘기되, ``__await__`` 프로토콜을 따르는 객체에만 사용할 수 있습니다.
...
``await``는 awaitable이 완료될 때까지 현재 코루틴을 중단하고, 완료 결과 또는 예외를 다시 받습니다. ``yield from``과 역사적·구조적 연관은 강하지만 둘은 같은 연산이 아닙니다. 이 중단과 재개를 포함해, *앞 단계의 결과로 다음 단계를 계속하는 ``async def``의 전체 흐름*이 우리가 배운 flatMap/bind와 대응합니다.
...
async def get_user_comment_count(user_id: int) -> int:
    ...
    # 의존적 합성 패턴을 async/await로 표현
    user = await fetch_user_async(user_id)
    posts = await fetch_posts_async(user["id"])
    comments = await fetch_comments_async(posts[0]["id"])
    return len(comments)
...
``await fetch++_++user++_++async(user++_++id)`` 하나는 awaitable의 결과를 얻는 표현입니다. 그 결과를 이어서 ``fetch++_++posts++_++async``에 넘기는 *이 함수 전체*는, ``Future.flatMap(lambda user: fetch_posts_async(user["id"]))``처럼 의존적인 계산을 합성하는 모양과 대응합니다. 단, Python의 코루틴·Task·Future는 취소, 예외, 스케줄링 같은 관찰 가능한 효과가 있으므로 수학적 Monad 법칙을 자동으로 만족한다고 결론 내리면 안 됩니다.

이 대응 덕분에 async/await는 콜백 중첩 없이 의존적 작업을 위에서 아래로 읽히게 합니다. 명시적인 Future 객체가 없더라도, 그 흐름을 Monad의 순차 합성이라는 렌즈로 이해할 수 있습니다.

____
*핵심 통찰: 의존적인 ``await`` 흐름은 ``flatMap``/bind와 대응한다*

이 장의 가장 중요한 결론입니다. ``await``는 마법이 아닙니다. awaitable을 중단·재개하는 언어 연산이며, ``async def`` 안에서 그 결과를 다음 비동기 단계로 넘기는 방식은 **모나드의 ``flatMap``/bind를 읽기 쉽게 표현한 것과 대응**합니다.

* ``Future.flatMap(lambda x: next_step(x))``
* ``yield from inner_generator()``
* ``value = await step(); await next_step(value)``


세 표현은 모두 "앞 계산의 결과로 다음 계산을 잇는다"는 문제를 조명합니다. 그러나 실행 모델은 다릅니다. ``yield from``은 제너레이터 위임이고, ``await``는 awaitable 프로토콜과 이벤트 루프의 스케줄링을 이용하며, 여기서 만든 ``Future.flatMap``은 학습용 추상화입니다. 공통된 합성의 원리를 보되, API를 서로 완전히 같은 것으로 취급하지 않는 것이 중요합니다.
____
...
*진화 2*: ``await`` - 의존적인 비동기 단계를 선형적으로 쓰는 문법

[source,python]
----
# 의존적인 비동기 단계를 await로 표현
...
- *4장 await* → 의존적인 비동기 단계를 "동기 코드처럼 읽히는 문법"으로 표현
...
Monad의 flatMap은 "한 계산이 끝나면 그 결과로 다음 계산을 시작한다"는 순차 합성을 표현합니다. ``yield from``과 의존적인 ``await`` 흐름은 이 합성의 모양을 언어 문법으로 자연스럽게 드러냅니다. 다만 Python의 취소·예외·스케줄링까지 포함한 API가 수학적 Monad 그 자체는 아닙니다.
...
crawl++_++page에서 각 링크를 크롤링할 때 asyncio.gather를 사용합니다. 이것은 여러 awaitable을 동시성으로 스케줄링하고, 결과를 모읍니다. 다음 장에서 배울 Applicative 관점과 연결됩니다.
...
class BadFuture:
    def flatMap(self, func):
        ...
        if self._completed:
            ...
            raise RuntimeError("이 예제에서는 미완료 Future를 생략합니다")
...
*교훈*: Monad 법칙을 리팩터링 근거로 삼고 싶다면 ``flatMap``과 ``map`` 자체의 관찰 가능한 의미를 순수하게 유지하세요. 로깅·파일 쓰기·전역 상태 변경을 그 안에 숨기면 법칙의 "같다"가 결과값만의 동치인지 로그까지 포함한 동치인지 모호해집니다.
...
셋째, 제너레이터의 ``yield from``과 ``async/await``는 모두 중단 가능한 계산을 선형적으로 읽히게 합니다. 의존적인 ``await`` 흐름은 flatMap/bind와 대응하지만, ``await`` 자체와 ``yield from``을 수학적 flatMap과 동일시하지는 않습니다.
...
다섯째, async/await는 명령형 문법으로도 함수형의 합성 관점을 활용하게 합니다. 코드는 위에서 아래로 읽히지만, 의존적인 단계의 연결을 bind로 해석할 수 있습니다. 이 해석이 곧 코드가 순수하거나 Monad 법칙을 만족한다는 보장은 아닙니다.
...
. *``await``의 정체*: ``await``는 awaitable을 중단·재개하는 언어 연산이며, 의존적인 ``async def`` 흐름은 **비동기 Monad의 ``flatMap``/bind와 대응**합니다.
...
둘째, ``await``는 순차 실행이므로 독립적인 작업들을 함께 진행하려면 동시성 합성이 필요합니다. 이는 **link:../06-concurrency-and-parallelism/[6장]**의 **Applicative 관점**과 ``asyncio.gather``에서 다룹니다.
// end::body[]
