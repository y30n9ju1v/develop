...
.초보자 이정표
====
*선수지식:* Python 함수 호출. ``asyncio``를 써 본 적은 없어도 됩니다.

*이번 장의 도착점:* ``await``가 기다리는 동안 다른 작업에 제어를 돌려주며, 독립 작업은 ``gather``로 함께 시작할 수 있음을 실행 시간으로 확인합니다.

*핵심 낱말:* 블로킹(blocking), I/O-bound, 코루틴(coroutine), 동시성(concurrency), 병렬성(parallelism). 이 장의 동시 실행은 CPU를 여러 코어에서 병렬 실행한다는 뜻이 아닙니다.
====

== 먼저 10줄로 실행해 보기

외부 API나 라이브러리 없이, 아래 파일 하나만 실행해 보세요. ``await``는 기다리는 동안 이벤트 루프에 제어를 돌려주고, ``gather``는 서로 독립적인 두 작업을 함께 시작합니다.
...
async def slow(label: str) -> str:
    await asyncio.sleep(0.1)  # 네트워크 응답을 기다린다고 가정한다
    return label
...
async def main() -> None:
    ...
    first = await slow("A")
    second = await slow("B")
    print(f"순차: {first}, {second} ({time.perf_counter() - started:.2f}초)")

    started = time.perf_counter()
    first, second = await asyncio.gather(slow("A"), slow("B"))
    print(f"동시: {first}, {second} ({time.perf_counter() - started:.2f}초)")

asyncio.run(main())
----

대체로 ``순차``는 약 0.20초, ``동시``는 약 0.10초가 출력됩니다. 첫 번째 경우는 두 기다림을 차례로 하고, 두 번째 경우는 두 기다림의 시간이 겹칩니다. 이 차이만 이해한 뒤 아래의 웹 API 예제로 넘어가세요.
...
def get_user_first_post_comments_async(user_id, callback):
    """콜백으로 결과를 전달하는 버전"""
    def on_user(err, user):
        if err:
            callback(err, None)
            return
        fetch_posts_async(user["id"], on_posts)

    def on_posts(err, posts):
        if err:
            callback(err, None)
            return
        if not posts:
            callback(None, [])
            return
        fetch_comments_async(posts[0]["id"], callback)

    fetch_user_async(user_id, on_user)

# 이것을 다른 함수와 조합하려면?
def process_user_data_async(user_id, callback):
    def on_comments(err, comments):
        if err:
            callback(err, None)
            return
        analyze_sentiment_async(comments, callback)

    get_user_first_post_comments_async(user_id, on_comments)
----

콜백도 이렇게 이름 붙인 함수로 조합할 수 있습니다. 문제는 합성 자체가 불가능하다는 것이 아니라, 각 단계가 콜백 규약과 에러 전달 방식을 다시 받아야 하며, 반환 타입으로 흐름을 드러내기 어렵다는 데 있습니다. 작업이 길어질수록 제어 흐름이 비즈니스 로직에서 분리되고, 취소·타임아웃·자원 정리까지 일관되게 전달하기 어려워집니다. 이것이 콜백 지옥의 본질입니다.
...
posts_future = user_future.flat_map(   # 의존적 작업 연결 - 합성 가능!
    lambda user: fetch_posts(user.id)
)
comments_future = posts_future.flat_map(  # 계속 합성!
...
콜백 스타일은 link:../01-functional-thinking/[1장]의 합성 원칙을 적용하기 어렵게 만듭니다:

- ⚠️ 합성은 가능하지만, 콜백 규약과 에러 전달이 모든 단계에 퍼짐
- ⚠️ 비동기 결과가 반환값이 아니라 제어 흐름으로 전달됨
- ⚠️ 실행 시점·스레드·취소 규약이 함수 시그니처에 드러나지 않음
...
def crawl_page(url):
    ...
    def on_page(err, html):
        ...
        for link in links:
            def on_link(err, html, link=link):
                if err:
                ...
                # 콜백이 끝없이 중첩됩니다...
            fetch_page_async(link, on_link)

    fetch_page_async(url, on_page)
