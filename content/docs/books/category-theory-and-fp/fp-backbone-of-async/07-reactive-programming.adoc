...
.초보자 이정표
====
*선수지식:* 리스트를 순회해 본 경험과 6장의 여러 비동기 작업. Reactive Programming 경험은 필요 없습니다.

*이번 장의 도착점:* 한 번 도착하는 값(Future)과 시간에 따라 여러 번 도착하는 값(스트림)을 구분하고, 각 이벤트를 순수한 변환 함수로 다룹니다.

*핵심 낱말:* 스트림(stream), 이벤트(event), Observable, 구독(subscription), backpressure. 라이브러리 API보다 먼저 "값 하나냐, 시간에 걸친 값들이냐"를 구분하세요.
====

== 1-6장 복습: 여기까지의 여정

*7장*을 시작하기 전에, 지금까지 배운 개념들을 정리해봅시다.
...
link:../06-concurrency-and-parallelism/[6장]에서는 ``Applicative`` 관점(``asyncio.gather``)으로 *독립적인* 비동기 작업을 **동시성**으로 실행하여 I/O 대기를 겹쳤습니다. 하지만 우리가 다룬 것은 여전히 "미래에 도착할 *하나의 값*"(``Future``)이었습니다.
...
[WARNING]
.RxPY로 넘어가면 백프레셔가 공짜로 따라오지 않습니다
====
방금 다룬 ``asyncio.Queue``의 백프레셔는 **Pull 기반**이라 자동이었습니다 — 소비자가 ``get()``을 호출해야만 값이 흐르므로, 소비자가 느리면 생산자가 ``put()``에서 자연히 멈춥니다. 그런데 RxPY의 ``Observable``은 기본적으로 **Push 기반**입니다 — 생산자가 소비자의 사정을 봐주지 않고 값을 계속 밀어 넣습니다. 소비자(구독자)가 처리 속도를 못 따라가면, 처리되지 못한 값들이 어딘가에 쌓이다가 메모리를 다 먹어치우는 OOM으로 이어질 수 있습니다. RxPY에서 이 문제를 다루려면 ``op.buffer_with_time()``, ``op.sample()``처럼 값을 일부러 버리거나 묶어서 유량을 줄이는 연산자를 명시적으로 끼워 넣어야 합니다 — 뒤에서 다룰 ``throttle``, ``debounce``도 결국 이 백프레셔 문제의 부분적인 해법입니다. Push 기반 스트림을 다룰 때는 "이 스트림, 소비자가 못 따라가면 어떻게 되지?"를 항상 먼저 물어야 합니다.
====
