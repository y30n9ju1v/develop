...
.초보자 이정표
====
*선수지식:* 1장의 순수 함수, 4장의 순차 합성. 웹 프레임워크 경험은 있으면 좋지만 필수는 아닙니다.

*이번 장의 도착점:* 의존성을 함수 인자로 주입하고, 미들웨어처럼 여러 단계를 조합해 I/O 경계와 비즈니스 규칙을 분리합니다.

*핵심 낱말:* 의존성 주입(dependency injection), 클로저(closure), 미들웨어(middleware), 경계(boundary). 대규모 아키텍처 전체를 외우지 말고 한 요청이 어떤 함수들을 거치는지만 추적하세요.
====
...
- 의존적인 async/await 흐름은 flatMap/bind와 대응
...
*link:../06-concurrency-and-parallelism/[6장]: Applicative Functor - 독립 작업의 동시성 결합*
