...
link:../08-practical-patterns/[8장]을 시작하기 전에, 지금까지 배운 핵심 개념들을 정리해봅시다. 우리는 link:../01-functional-thinking/[1장]부터 link:../07-reactive-programming/[7장]까지 함수형 프로그래밍의 이론적 토대를 닦았습니다. ``map``, ``flatMap``, ``Result``, ``Applicative``, 그리고 ``Observable``까지.
...
link:../08-practical-patterns/[8장]의 ``Transaction``, ``Request.context``는 **가변 상태**를 가집니다. 이것이 FP 원칙과 충돌하는 것처럼 보일 수 있습니다.
