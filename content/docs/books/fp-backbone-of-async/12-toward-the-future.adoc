...
*실무 주의보: 파이썬에는 꼬리 재귀 최적화(TCO)가 없습니다.* 위 ``flow`` 예제처럼 ``bind``/``map``을 **순차적으로 이어붙이는 체이닝**은 각 호출이 완료되고 반환된 뒤 다음 호출이 시작되므로 스택이 누적되지 않습니다 — 체인이 아무리 길어도 안전합니다. 위험한 건 이런 순차 체이닝이 아니라, 하스켈이나 스칼라에서 흔히 쓰는 **재귀적으로 정의된 순회/폴드**를 파이썬으로 그대로 옮길 때입니다. 예를 들어 긴 리스트를 한 원소씩 재귀 호출로 처리하며 ``Result``를 누적하는 함수를 쓰면, 리스트 길이만큼 콜스택이 쌓여 파이썬의 재귀 한도(기본 1000)를 금방 넘길 수 있습니다. 5장의 ``sequence_results``가 재귀 대신 ``for`` 루프로 ``Result`` 리스트를 순회했던 게 우연이 아닙니다 — TCO가 있는 언어라면 재귀로 표현주고 넘어갈 부분을, 파이썬에서는 처음부터 반복문으로 짜야 스택 오버플로우를 피할 수 있습니다. 정말 재귀적 구조(트리 순회 등)를 다뤄야 한다면, 재귀를 명시적인 스택으로 흉내 내는 **트램펄린(trampoline)** 기법으로 우회하는 방법도 있습니다.
