...
*주의: ``map``이 삼켜도 되는 예외는 없습니다.* 초기 버전에서 ``func(self._value)``를 ``try``로 감싸 "함수 실행 중 아무 예외가 나든 조용히 ``Maybe(None)``으로 바꾼다"는 유혹을 느낄 수 있습니다. 이건 안티패턴입니다 — ``Maybe``가 표현하려는 건 "값이 있을 수도, 없을 수도 있다"는 **부재(absence)**이지, "계산이 실패할 수도 있다"는 **실패(failure)**가 아닙니다. 이 둘을 구분하지 않고 ``except Exception``으로 모든 예외를 흡수하면, 정상적인 부재("주소가 없는 사용자")와 진짜 버그(``addr.get`` 호출에서 오타로 난 ``TypeError``)가 똑같이 ``Maybe(None)``으로 뭉개져서 디버깅이 거의 불가능해집니다. 함수 실행이 실패할 수 있는 경우를 명시적으로 표현하고 싶다면, 그건 ``Maybe``가 아니라 link:../05-error-handling-as-types/[5장]에서 다룰 ``Result`` 타입의 역할입니다.
