...
.초보자 이정표
====
*선수지식:* 5.1절의 명령어와 5.4절의 평가 과정. 이 절은 5부의 심화 읽기입니다.

*이번 장의 도착점:* 소스 표현식을 실행하지 않고 명령어열로 바꾸며, 생성된 코드에서 불필요한 저장·복원을 줄이는 관점을 익힙니다.

*핵심 낱말:* 컴파일(compile), 목적지(target), 연결(linkage), 명령어열(instruction sequence), 꼬리 호출. ``compile``과 ``compileM``은 완결 구현이 아닌 설계 스케치이므로, 자료 흐름을 읽는 데만 사용하세요.
====
