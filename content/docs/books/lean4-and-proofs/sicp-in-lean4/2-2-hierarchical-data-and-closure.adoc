...
.초보자 이정표
====
*선수지식:* 2.1절의 생성자와 선택자, 리스트의 "앞 원소와 나머지"라는 생각.

*이번 장의 도착점:* 리스트와 트리처럼 중첩된 데이터를 재귀로 순회하고, 연산이 새 자료형으로 자연스럽게 확장되는 모습을 봅니다.

*핵심 낱말:* 리스트(List), 트리(Tree), 구조적 재귀(structural recursion), 닫힘 성질(closure property). 그림 언어 관련 확장은 이 책의 범위 밖이므로 건너뛰어도 됩니다.
====
