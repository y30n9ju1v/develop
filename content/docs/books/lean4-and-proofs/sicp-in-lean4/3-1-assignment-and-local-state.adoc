...
.초보자 이정표
====
*선수지식:* 2부의 자료형과 함수. ``IO``는 이 장에서 처음 만나도 됩니다.

*이번 장의 도착점:* ``IO.Ref``라는 변경 가능한 상자를 만들고 읽고 쓰며, 같은 함수 호출이 이전 호출의 영향을 받을 수 있음을 확인합니다.

*핵심 낱말:* 상태(state), 참조(reference), 효과(effect), 참조 투명성. ``partial``의 종료성 논의는 첫 읽기에서는 "이 루프가 끝난다고 Lean이 보장할 수 없다"로만 이해해도 됩니다.
====
...
[source,text]
----
 [참조 투명성(Referential Transparency)의 붕괴]

 1장/2장 (순수 함수):
   f(25) ===> 항상 75  (언제, 몇 번 호출하든 불변!)

 3장 (배정과 가변 상태):
   withdraw 25 ──(1회차)──► .ok 75   [balance: 100 ↦ 75]
   withdraw 25 ──(2회차)──► .ok 50   [balance:  75 ↦ 50]
   withdraw 60 ──(3회차)──► .error   [balance:  50 유지]
   withdraw 15 ──(4회차)──► .ok 35   [balance:  50 ↦ 35]

   ==> 동일한 표현식 `withdraw 25`가 호출된 '시점(시간)'에 따라 다른 값을 반환!
----
...
[source,text]
----
 [makeWithdraw가 생성한 독립된 두 객체의 힙 메모리 구조]

   let w1 ← makeWithdraw 100        let w2 ← makeWithdraw 100
            │                                │
            ▼                                ▼
   ┌──────────────────┐             ┌──────────────────┐
   │  w1 클로저 함수  │             │  w2 클로저 함수  │
   │  code: fun ...   │             │  code: fun ...   │
   │  env: balance ───┼──┐          │  env: balance ───┼──┐
   └──────────────────┘  │          └──────────────────┘  │
                         ▼                                ▼
                 ┌───────────────┐                ┌───────────────┐
                 │ IO.Ref (힙)   │                │ IO.Ref (힙)   │
...
[source,text]
----
 [물리적 가변성 (IO.Ref) vs 논리적 가변성 (StateM)]

 1. IO.Ref (물리적 가변성 / SICP 모델):
    - 힙 메모리에 고유한 정체성을 가진 셀 할당
    - 클로저가 암묵적으로 셀을 포획하여 독립적 상태 유지
    - 단점: 참조 투명성 붕괴, IO 모나드 전파, 종료성 추적 불가

 2. StateM (논리적 가변성 / 순수 함수형 모델):
    - σ -> (α × σ) 순수 함수 파이프라인
    - 명시적으로 상태 값을 함수 간에 실 꿰듯(threading) 전달
    - 장점: 참조 투명성 유지, 순수성 보장
    - 단점: 다중 객체 독립 상태 관리 시 배관(plumbing) 코드 폭증
----
