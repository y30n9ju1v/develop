...
.초보자 이정표
====
*선수지식:* 1.1절의 ``let``·함수 호출, 3.1절의 상태와는 구별되는 "이름의 범위"라는 생각.

*이번 장의 도착점:* 변수 이름이 값 자체가 아니라 환경에서 찾는 연결이며, 함수가 만들어진 위치의 환경을 기억한다는 사실을 그림과 코드로 추적합니다.

*핵심 낱말:* 환경(environment), 프레임(frame), 스코프(scope), 클로저(closure). 환경 모델은 실행 순서를 설명하는 도구이지 Lean 런타임의 내부 구현을 그대로 복사한 것은 아닙니다.
====
...
[source,text]
----
 [환경 모델의 프레임 사슬 (Frame Chain)]

 ┌────────────────────────────────────────────────┐
 │ 전역 환경 (Global Environment)                │
 │  makeAdder: (코드)                             │
 │  addFive:   (클로저: code, env ──► Frame E1)   │
 │  addTen:    (클로저: code, env ──► Frame E2)   │
 └───────────────────────▲────────────────────────┘
                         │
          ┌──────────────┴──────────────┐
          │                             │
...
[source,text]
----
 [가변 상태 클로저(makeWithdraw)의 환경 모델]
 ...
 └───────────────────────────▲────────────────────────────┘
     ...
     ┌───────────────────┐         ┌───────────────────┐
     │ 프레임 E1         │         │ 프레임 E2         │
     │  balance: [100]───┼──(Ref)  │  balance: [100]───┼──(Ref)
     └───────────────────┘         └───────────────────┘
----
